...
/mob/living/carbon/human/proc/create_contract_pr(var/newname = "none", var/newdesc = "none", var/newprice = "none")
	...
	if (newname != null && newname != "none" && newdesc != null && newdesc != "none" && newprice != null && newprice != "none")
		// Check if contract name already exists
		if(newname in GLOB.custom_contracts_names)
			to_chat(usr, span_warning("A contract with that name already exists!"))
			return

		// Add contract to global lists
		GLOB.custom_contracts_names += newname
		GLOB.custom_contracts[newname] = list(H, newdesc, newprice) // owner, description, price
...
/// Check all contracts (for debugging)
